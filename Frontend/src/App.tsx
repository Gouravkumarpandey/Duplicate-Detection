...
function App() {
  ...
  const [showAddToDbButton, setShowAddToDbButton] = useState(false);
  const [isSavingToDb, setIsSavingToDb] = useState(false);

  const handleFolderSelect = useCallback(async (selectedFiles: FileList) => {
    setIsScanning(true);
    setShowAddToDbButton(false); // Reset button when starting new scan
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    try {
      ...
      // Show "Add to DB" button after successful deletion
      setShowAddToDbButton(true);
  ...
  const handleAddToDb = useCallback(async () => {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to add ${files.length} files to the database?`
    );
    ...
    try {
      // Convert files to serializable format by excluding the File object
      ...
      })).filter(file => file.path && file.name && file.hash); // Filter out invalid files
      
      console.log('Sending files to database:', serializableFiles);
      
      if (serializableFiles.length === 0) {
        alert('No valid files to save to database.');
        return;
      }
      
      const response = await apiService.saveFilesToDB(serializableFiles);
      
      console.log('Database response:', response);
      
      // Hide the "Add to DB" button after successful save
      setShowAddToDbButton(false);
      
      if (response.failureCount > 0) {
        alert(`Successfully saved ${response.successCount} files to database. ${response.failureCount} files failed to save.`);
      } else {
        alert(`Successfully saved ${serializableFiles.length} files to database.`);
      }
    } catch (error) {
      console.error('Error saving files to database:', error);
      alert('Error saving files to database. Please try again.');
    } finally {
              ...
              <div className="flex items-center gap-3">
                {showAddToDbButton && (
                  <button
                    ...
                    disabled={isSavingToDb || files.length === 0}
                    className="flex items-center gap-2 px-3 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
                  >
                    <Database size={16} />
                    {isSavingToDb ? 'Saving...' : `Add to DB (${files.length})`}
                  </button>
                )}
